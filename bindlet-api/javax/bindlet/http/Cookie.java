 ...
 * browser, and later sent back to the server. A cookie's value can uniquely identify a client, so
 ...
 * the optional attributes, so use them sparingly to improve the interoperability of your servlets.
 * 
 * <p>
 * The servlet sends cookies to the browser by using the {@link IHttpBindletResponse#addCookie}
 * method, which adds fields to HTTP response headers to send cookies to the browser, one at a time.
 * The browser is expected to support 20 cookies for each Web server, 300 cookies total, and may
 * limit cookie size to 4 KB each.
 * 
 * <p>
 * The browser returns cookies to the servlet by adding fields to HTTP request headers. Cookies can
 * be retrieved from a request by using the {@link IHttpBindletRequest#getCookies} method. Several
 ...
 * cookies created with this class. This class does not support the cache control defined with HTTP
 * 1.1.
 * 
 * <p>
